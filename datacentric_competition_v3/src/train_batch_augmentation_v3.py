...
class BatchAugmentation:
    ...
    def __init__(self, parent_dir, tar_ext="_generated_dataset", test_path="label_book"):
    ...
    def sort_out_val(self):
        ...
        equi_train_sample = int(np.ceil(train_sample/32) * 32)
        ...
        val_needed_actual = int(np.floor(val_needed / 32) * 32)
    ...
    def __call__(self, *args, **kwargs):
        ...
        for kk, vv in self.src_images_path_dict.items():
            ...
            if kk in ['i']:
                ...
                hard_case_list = self.hard_case_path_dict[kk]
                hard_case_count = len(hard_case_list)
                shuffle(vv)
                list(map(self.apply_transform_affine_full, vv[hard_case_count:]))
                shuffle(vv)
                list(map(functools.partial(self.apply_transform_shear, lamda=lamda), vv[32:]))
                list(map(self.sort_out_hard_case, hard_case_list))
            else:
                hard_case_list = self.hard_case_path_dict[kk]
                hard_case_count = len(hard_case_list)
                list(map(functools.partial(self.apply_transform_scale, s_x=s_x, s_y=s_y), vv))
                shuffle(vv)
                list(map(self.apply_transform_affine_left, vv[int(hard_case_count/2):]))
                shuffle(vv)
                list(map(self.apply_transform_affine_full, vv[int(hard_case_count/2):]))
                shuffle(vv)
                list(map(functools.partial(self.apply_transform_shear, lamda=lamda), vv[32:]))
                list(map(self.sort_out_hard_case, hard_case_list))
    ...
    def apply_transform_affine_full(self, src_img):
        ...
        tar_img_name = img_name.split('.')[0] + f"_{trans_fname}.png"
...
if __name__ == "__main__":
    batch_aug = BatchAugmentation(parent_dir="D:/datacentric_demo/data", tar_ext="_sub_11Oct")
    batch_aug()