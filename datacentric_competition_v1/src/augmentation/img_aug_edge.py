...
target_dir = source_dir + "_augmented_edge"
...
def augment_images(img):
    ...
    #img_ = cv2.erode(img, kernel, iterations=3)
    #img_ = cv2.dilate(img, kernel, iterations=1)
    #img_ = img + 255 - (np.random.rand(*(img.shape)))
    #img_ = cv2.Sobel(img, ddepth=cv2.CV_64F, dx=1, dy=0, ksize=3)
    img_ = cv2.Canny(img, 10, 43)
    x = np.random.randint(2)
    img_ = img_ if x == 0 else 255 - img_
    image = (img_ // 43) * 43
    image[image > 43] = 255
    return image
