...
from sample_count import get_auto_class_weight, get_sample_class_weight
...
if __name__ == "__main__":
   ...
   #     lambda image, label: (tf.image.random_flip_left_right(image), label))
    ...
    if not args.eval:
        ...
        def get_pd_dict(ds: tf.data.Dataset, vis=False, tt='val'):
            ...
            cm = np.round(np.divide(cm, cm.sum(axis=-1, keepdims=True) + 1e-10), 1)
        ...
        # val_pd_dict, val_acc = get_pd_dict(valid, vis=True, tt='val')
        ...
        #train_pd_dict, train_acc = get_pd_dict(train_)
        test_pd_dict, test_acc = get_pd_dict(test_, vis=True, tt='test')
