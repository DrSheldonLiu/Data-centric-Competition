...
class BatchAugmentation:
    ...
    def __call__(self, *args, **kwargs):
        ...
        for kk, vv in self.src_images_path_dict.items():
            if kk != 'i' and kk != 'ii' and kk != 'iii':
                continue
            print(f'processing {kk}')
            if kk in ['i', 'ii', 'iii', 'x']:
                # random do the augmentation - mirror, fliph, rotate, invert, crop, morphological, edge, noise, add_dot_lines
                method_list = [f"augment_{i}" for i in ['add_dot_lines', 'crop', 'fliph', 'fliplr', 'invert', 'mor', 'rotate']]
                for each_vv in vv[:30]:
                    selected_method = random.sample(method_list, k=random.choice([3, 4]))
                    #selected_method = random.sample(method_list, k=random.choice([2, 3, 4]))
                ...
                method_list = [f"augment_{i}" for i in ['add_dot_lines', 'crop', 'iv_enrich_vi', 'invert', 'mor', 'rotate']]
                for each_vv in vv:
                    selected_method = random.sample(method_list, k=random.choice([3, 4]))
                    for mm in selected_method:
                        self.__getattribute__(mm)(each_vv)
                ...
                method_list = [f"augment_{i}" for i in ['add_dot_lines', 'crop', 'vi_enrich_iv', 'invert', 'mor', 'rotate']]
                for each_vv in vv:
                    selected_method = random.sample(method_list, k=random.choice([3, 4]))
                    for mm in selected_method:
                        self.__getattribute__(mm)(each_vv)
                ...
                method_list = [f"augment_{i}" for i in ['add_dot_lines', 'crop', 'fliplr', 'invert', 'mor', 'rotate']]
                for each_vv in vv:
                    selected_method = random.sample(method_list, k=random.choice([3, 4]))
                    for mm in selected_method:
                        self.__getattribute__(mm)(each_vv)
                ...
                method_list = [f"augment_{i}" for i in ['add_dot_lines', 'crop', 'invert', 'mor', 'noise', 'rotate']]
                for each_vv in vv:
                    selected_method = random.sample(method_list, k=random.choice([3, 4]))
                    for mm in selected_method:
                        self.__getattribute__(mm)(each_vv)
                ...
                method_list = [f"augment_{i}" for i in ['fliph', 'add_dot_lines', 'crop', 'invert', 'mor', 'noise', 'rotate']]
                for each_vv in vv:
                    selected_method = random.sample(method_list, k=random.choice([3, 4]))
                    for mm in selected_method:
                        self.__getattribute__(mm)(each_vv)
...
if __name__ == "__main__":
    batch_aug = BatchAugmentation(parent_dir="D:/datacentric_demo/data")
    batch_aug()