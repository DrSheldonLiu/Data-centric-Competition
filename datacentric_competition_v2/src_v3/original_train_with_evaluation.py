...
directory = "../../datacentric_competition_v3/submission_2708"
user_data = directory
...
if __name__ == "__main__":
    ...
    if not args.eval:
        ...
        print('evaluation ->')
        model.load_weights("../../datacentric_competition_v3/src/submission_1208/best_model")
        parent_dir = "D:/tars"
        performance_df_list = list()
        for sub_folder in os.listdir(parent_dir):
            print('processing ', sub_folder)
            user_data = os.path.join(parent_dir, sub_folder)
            ...
            val_pd_dict, val_acc = get_pd_dict(train_, vis=False, tt='val', show_detail=False)
            print('-------------------------------------')
            print(sub_folder)
            print('validation ->')
            ...
            print('\n')
            p_df = pd.DataFrame(val_pd_dict, index=[0])
            p_df['dataset_name'] = sub_folder
            performance_df_list.append(p_df)
        df = pd.concat(performance_df_list, axis=0)
        df = df[[df.columns.tolist()[-1]] + df.columns.tolist()[:-1]]
        df.to_csv('performance_df.tsv', sep='\t', index=False)

            # test_pd_dict, test_acc = get_pd_dict(test, vis=True, tt='label_book', show_detail=False)
            # print('-------------------------------------')
            # print('test ->')
            # print(f'overall acc : {test_pd_dict}')
            # print(f'per class acc: {test_acc}')
            # print('\n')
